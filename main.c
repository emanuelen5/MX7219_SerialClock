...
void initSPI(void) {
	...
	SPCR |= BV(SPE) | BV(MSTR) | BV(SPR1) | BV(SPR0); // Enable SPI | Master device | Clock/128
	//SPSR |= BV(SPI2X); // Double clock speed
...
int main(void)
{
	...
	MX(MX_POWER, MX_ON);
	MX(MX_SCAN_LIMIT, 0xFF);
	MX(MX_DISPLAY_TEST, MX_ON);
	MX(MX_DIGIT0, 1);
	MX(MX_DIGIT1, 2);
	MX(MX_DIGIT2, 3);
